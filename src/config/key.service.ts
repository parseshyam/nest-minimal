...
export class KeysService {
    ...
    private checkEnv = () => {
        if (!process.env.NODE_ENV) {
            ...
            console.log(`Invalid enviroment specified allowed enviroments are ${this.defaultEnv.join(", ")}`)
            process.exit(1);
        }
        const isEnvExists = fs.existsSync(`.${process.env.NODE_ENV}.env`);
        if (!isEnvExists) {
            console.log(`Enviroment file is missing please add .${process.env.NODE_ENV}.env in your root dir.`);
            process.exit(1);
        }
    }
    ...
    private checkEnvKeys = () => {
        ...
        FCM.private_key = process.env[`${ENV}_private_key`]?.replace(/\\n/g, '\n') || "";
        ...
        FCM.auth_provider_x509_cert_url = process.env[`${ENV}_auth_provider_x509_cert_url`];
        ...
        if (missingEnvKeys.length) {
            console.log(`Missing Env Keys: Please copy and set following keys in .${process.env.NODE_ENV}.env file \n \n`);
